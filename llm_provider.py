...
TOOL_FRIENDLY_DEFAULT = "cohere.command-a-03-2025"
...
def make_llm():
    ...
    if provider == "oci":
        ...
        max_ct = int(
            os.getenv("OCI_MAX_COMPLETION_TOKENS", os.getenv("OCI_MAX_TOKENS", "1024"))
        )
        model_kwargs = {"temperature": temperature, "max_tokens": max_ct}

        print(
            f"[llm_provider] Using provider={provider_tag} model={model_id} kwargs={model_kwargs}"
        )
        return ChatOCIGenAI(
            model_id=model_id,
